...
parser.add_argument("--weather_dir", type=str, default=None,
                    help="Directory containing weather CSVs (e.g. san.csv, sfo.csv, lax.csv)")
...
# ================================================================
# Weather utilities: load / clean / merge
# ================================================================
def load_weather_data(weather_dir):
    """
    从 weather_dir 中读取所有天气 csv（san.csv, sfo.csv, lax.csv...）
    返回:
      weather_all: 包含所有机场的天气数据
      weather_feature_cols: 用作特征的列名（不含 IATA/NAME/DATE/date）
    """
    ...
    if len(csv_files) == 0:
        print(f"⚠️ No weather CSV files found in {weather_dir}")
        return None, []
    ...
    for fpath in csv_files:
        try:
            ...
            # 统一列名为大写，避免大小写问题
            dfw.columns = [c.upper().strip() for c in dfw.columns]

            # 处理 IATA 列：有就用，没有就从文件名推断（如 san.csv → SAN）
            if "IATA" not in dfw.columns:
                iata_from_name = os.path.basename(fpath).split(".")[0].upper()  # san.csv → SAN
                dfw["IATA"] = iata_from_name

            # 确保有 DATE 列（你的文件里已经有了）
            if "DATE" not in dfw.columns:
                raise ValueError(f"{fpath} has no DATE column")

            # 统一日期格式：转成 datetime.date
            dfw["date"] = pd.to_datetime(dfw["DATE"]).dt.date

            # 选择真正作为特征的列：去掉 IATA/NAME/DATE/date
            drop_cols = {"IATA", "NAME", "DATE", "STATION"}
            weather_feature_cols = [c for c in dfw.columns
                                    if c not in drop_cols and c != "date"]

            # 保证这些列是数值（非数值转成 NaN）
            for c in weather_feature_cols:
                dfw[c] = pd.to_numeric(dfw[c], errors="coerce")

            dfs.append(dfw[["IATA", "date"] + weather_feature_cols])
        except Exception as e:
            print(f"⚠️ Error loading weather file {fpath}: {e}")
    ...
    # 统一 IATA 大写
    weather_all["IATA"] = weather_all["IATA"].astype(str).str.upper().str.strip()

    # 从第一个 df 拿到特征列名（所有文件结构一样的话就是统一的）
    feature_cols = [c for c in weather_all.columns if c not in ["IATA", "date"]]

    print(f"✅ Loaded weather for {weather_all['IATA'].nunique()} airports, "
          f"{len(weather_all)} rows, {len(feature_cols)} feature cols")
    return weather_all, feature_cols


def merge_weather_into_flights(df_ca, weather_all, weather_feature_cols):
    """
    把天气特征根据 (IATA, date) merge 到 df_ca 上：
      - origin 机场天气：列名加 _ORIGIN 后缀
      - dest   机场天气：列名加 _DEST   后缀
    返回：
      df_ca_new, origin_weather_cols, dest_weather_cols
    """
    if weather_all is None or len(weather_feature_cols) == 0:
        return df_ca, [], []

    df_ca = df_ca.copy()

    # 统一航班日期
    df_ca["date"] = pd.to_datetime(df_ca["FL_DATE"]).dt.date

    # 准备 origin / dest 用的两个天气表
    weather_origin = weather_all[["IATA", "date"] + weather_feature_cols].copy()
    weather_dest = weather_all[["IATA", "date"] + weather_feature_cols].copy()

    origin_cols_map = {c: f"{c}_ORIGIN" for c in weather_feature_cols}
    dest_cols_map = {c: f"{c}_DEST" for c in weather_feature_cols}

    weather_origin.rename(columns=origin_cols_map, inplace=True)
    weather_dest.rename(columns=dest_cols_map, inplace=True)
    ...
    if "IATA" in df_ca.columns:
        df_ca = df_ca.drop(columns=["IATA"])  # merge 后多出来的 IATA 列删掉
    ...
    if "IATA" in df_ca.columns:
        df_ca = df_ca.drop(columns=["IATA"])

    # 删掉可能出现的重复列（比如 date_DROP 之类）
    drop_cols = [c for c in df_ca.columns if c.endswith("_DROP")]
    if drop_cols:
        df_ca = df_ca.drop(columns=drop_cols)

    # 填补 NaN（可以先简单填 0，后面你愿意可以改成均值/中位数）
    origin_weather_cols = list(origin_cols_map.values())
    dest_weather_cols = list(dest_cols_map.values())
    df_ca[origin_weather_cols + dest_weather_cols] = df_ca[origin_weather_cols + dest_weather_cols].fillna(0.0)

    print(f"🔗 Merged weather into flights: origin+dest features = {len(origin_weather_cols) + len(dest_weather_cols)}")
    return df_ca, origin_weather_cols, dest_weather_cols
...
# ================================================================
# 4.5 Merge weather features (optional)
# ================================================================
weather_all, weather_base_cols = load_weather_data(args.weather_dir)
df_ca, origin_weather_cols, dest_weather_cols = merge_weather_into_flights(
    df_ca, weather_all, weather_base_cols
)

df_ca.head(50).to_csv("merged_preview.csv", index=False)
print("Saved merged preview to merged_preview.csv")
# ================================================================
